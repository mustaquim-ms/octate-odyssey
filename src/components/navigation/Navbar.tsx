...
function NavbarContent() {
  ...
  useEffect(() => {
    ...
    if (authRequest === "login") { setAuthMode("login"); setIsAuthOpen(true); }
    else if (authRequest === "signup") { setAuthMode("signup"); setIsAuthOpen(true); }
  }, [searchParams]);
    ...
    <>
      <nav className="fixed top-0 w-full z-50 border-b border-[#7ed957]/10 bg-[#020617]/80 backdrop-blur-2xl">
        <div className="max-w-[1600px] mx-auto px-6 md:px-10 h-20 md:h-28 flex items-center justify-between">
          ...
          {/* PC MENU WITH SLIDING GLASS HOVER */}
            ...
            {menuItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {/* ACTIVE PILL */}
                  {isActive && (
                    <motion.div layoutId="nav-active" className="absolute inset-0 bg-[#7ed95710] border border-[#7ed95722] rounded-full -z-10" />
                  )}

                  {/* GLASS HOVER PILL (Sliding effect) */}
              ...
              <div className="hidden xl:block relative">
                 <button onClick={() => setShowProfile(!showProfile)} className="flex items-center gap-4 bg-white/5 border border-white/10 rounded-full p-2 pr-6 hover:bg-white/10 transition cursor-pointer">
                    <div className="w-12 h-12 rounded-full border-2 border-[#7ed957] overflow-hidden p-1">
                      <div className="w-full h-full rounded-full bg-gradient-to-br from-[#ffb423] to-[#7ed957] flex items-center justify-center text-black font-black uppercase">U</div>
                    </div>
                    <ChevronDown size={14} className={`text-gray-500 transition-transform ${showProfile ? 'rotate-180' : ''}`} />
                 </button>
                 {/* PC Dropdown menu here (truncated for space) */}
              </div>
            )}
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[115] bg-[#020617]/95 backdrop-blur-3xl xl:hidden flex flex-col pt-32 px-8"
          >
              ...
              {menuItems.map((item, idx) => (
                <motion.div key={item.name} initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: idx * 0.05 }}>
                  <Link 
                    ...
                    className={`flex items-center justify-between p-6 rounded-3xl border transition-all ${pathname.startsWith(item.href) ? 'bg-[#7ed95711] border-[#7ed95744] text-[#7ed957]' : 'bg-white/[0.02] border-white/5 text-white'}`}
                  >
                    <div className="flex items-center gap-4">
                      {item.icon}
                      <span className="font-[family-name:var(--font-outfit)] font-black uppercase tracking-widest text-lg">{item.name}</span>
                    </div>
                  </Link>
                </motion.div>
              ))}
