...
export default function Podium() {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-end max-w-5xl mx-auto">
      {/* 2nd Place */}
      <PodiumSlot Navigator={TOP_3[1]} height="h-48" delay={0.2} />
      
      {/* 1st Place */}
      <PodiumSlot Navigator={TOP_3[0]} height="h-64" delay={0} isGold />
      
      {/* 3rd Place */}
      <PodiumSlot Navigator={TOP_3[2]} height="h-40" delay={0.4} />
    </div>
  );
}

function PodiumSlot({ Navigator, height, delay, isGold }: any) {
    return (
        <motion.div 
        ...
        >
            <div className="mb-6 relative">
                <div className={`w-24 h-24 rounded-full bg-white/5 border-2 flex items-center justify-center relative z-10 
                    ...
                    <span className="font-black text-2xl uppercase font-[family-name:var(--font-outfit)]">{Navigator.name[0]}</span>
                </div>
            ...
            <div className={`w-full ${height} bg-[#0a101f] border-t-4 rounded-t-3xl p-6 flex flex-col items-center justify-center transition-all group-hover:bg-[#0a101f]/80`}
                 style={{ borderColor: Navigator.color }}>
                <p className="font-black text-xl mb-1 text-white uppercase tracking-tighter">{Navigator.name}</p>
                <p className="font-mono text-[9px] uppercase font-black tracking-widest mb-4" style={{ color: Navigator.color }}>{Navigator.rank}</p>
